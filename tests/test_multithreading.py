...
from src.tester import _wait_for_port
...
from src.main import process_subscription, deduplicate, filter_exit_idc_failed
...
class TestRelayTaskIsolation:
    ...
    async def test_concurrent_task_isolation_no_result_bleed(self, make_node, make_mock_process):
        ...
        async def mock_query_exit(port, timeout):
            idx = port - BASE_PORT
            return {"ip": f"1.2.3.{idx}", "isp": f"ISP-{idx}", "country": "US"}
        ...
        with (
            ...
            patch("src.tester._wait_for_port", new_callable=AsyncMock, return_value=True),
            patch("src.tester._query_exit_info", side_effect=mock_query_exit),
...
class TestQueryExitInfo:
    ...
    async def test_query_exit_info_primary_success(self, make_mock_process):
        """ip-api.com returns valid data on first try (curl fallback path)."""
        body = json.dumps({
            "status": "success",
            "query": "5.6.7.8",
            "isp": "TestISP",
        ...
        with (
            patch("asyncio.create_subprocess_exec", side_effect=mock_subprocess),
            patch("src.tester.HAS_SOCKS", False),
        ):
    ...
    async def test_query_exit_info_fallback_urls(self, make_mock_process):
        """Falls through to fallback URLs when primary fails (curl fallback path)."""
        call_count = 0
    ...
    async def test_query_exit_info_all_fail(self, make_mock_process):
        """Returns None when all endpoints fail (curl fallback path)."""
        async def mock_subprocess(*args, **kwargs):
...
class TestExitIdcFilter:
    """Unit tests for filter_exit_idc_failed."""

    def test_keeps_nodes_with_exit_isp(self, make_node):
        """Nodes with exit_isp are always kept."""
        n = make_node(name="ok", server="s1.com")
        n.exit_isp = "AWS"
        n.entry_country = "US"
        assert len(filter_exit_idc_failed([n])) == 1

    def test_discards_no_exit_isp_non_cn_entry(self, make_node):
        """Nodes without exit_isp and non-CN entry are discarded."""
        n = make_node(name="bad", server="s1.com")
        n.exit_isp = None
        n.entry_country = "US"
        assert len(filter_exit_idc_failed([n])) == 0

    def test_keeps_no_exit_isp_cn_entry(self, make_node):
        """Nodes without exit_isp but CN entry are kept."""
        n = make_node(name="cn", server="s1.com")
        n.exit_isp = None
        n.entry_country = "China"
        assert len(filter_exit_idc_failed([n])) == 1

    def test_keeps_no_exit_isp_no_country(self, make_node):
        """Nodes without exit_isp and no entry_country are discarded."""
        n = make_node(name="unknown", server="s1.com")
        n.exit_isp = None
        n.entry_country = None
        assert len(filter_exit_idc_failed([n])) == 0

    def test_mixed_scenario(self, make_node):
        """Mixed: keep CN entry without ISP, discard non-CN without ISP, keep all with ISP."""
        n1 = make_node(name="cn-no-isp", server="s1.com")
        n1.exit_isp = None
        n1.entry_country = "China"

        n2 = make_node(name="us-no-isp", server="s2.com")
        n2.exit_isp = None
        n2.entry_country = "US"

        n3 = make_node(name="us-has-isp", server="s3.com")
        n3.exit_isp = "AWS"
        n3.entry_country = "US"

        result = filter_exit_idc_failed([n1, n2, n3])
        assert len(result) == 2
        names = [n.name for n in result]
        assert "cn-no-isp" in names
        assert "us-has-isp" in names
...
class TestEndToEndPipeline:
    ...
    async def test_e2e_pipeline_all_tests_fail(self, make_node):
        ...
        # All nodes failed but kept (entry is CN, exit_isp=None is allowed)
        for n in nodes:
            assert n.test_success is False
